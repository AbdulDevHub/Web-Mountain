 ...
 * function the app uses, and reports top-1 / top-5 accuracy plus a list of
...
async function main() {
  ...
  for (const { input, expected } of EVAL_CASES) {
    ...
    if (emojis.includes(expected)) top5++;
    else misses.push({ input, expected, got: emojis });
  }

  const n = EVAL_CASES.length;
  console.log(`\nTop-1 accuracy: ${top1}/${n} (${((top1 / n) * 100).toFixed(1)}%)`);
  console.log(`Top-5 accuracy: ${top5}/${n} (${((top5 / n) * 100).toFixed(1)}%)\n`);

  if (misses.length) {
    console.log(`Missed cases (expected emoji not in top 5):`);
